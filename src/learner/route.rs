...
impl RouteLearner {
    ...
    pub fn select_route(&mut self, intent: QueryIntent, now_ms: u64) -> (u64, Route) {
        let routes = [Route::Semantic, Route::Keyword, Route::Temporal, Route::Artifact, Route::Hybrid, Route::Full];
        let seed = now_ms ^ (self.next_episode_id * 2654435761);

        let best = routes.iter().max_by(|a, b| {
            let sa = self.arms.entry((intent.clone(), (*a).clone())).or_insert(BetaPrior::new()).sample(seed);
            let sb = self.arms.entry((intent.clone(), (*b).clone())).or_insert(BetaPrior::new()).sample(seed.wrapping_add(1));
    ...
    pub fn feedback(&mut self, episode_id: u64, reward: f32) {
        if let Some((intent, route)) = self.pending.remove(&episode_id) {
            self.arms.entry((intent, route)).or_insert(BetaPrior::new()).update(reward);
        }
    }
    ...
    pub fn best_route(&self, intent: &QueryIntent) -> Route {
        ...
        routes.iter().max_by(|a, b| {
            let sa = self.arms.get(&(intent.clone(), (*a).clone())).map(|p| p.mean()).unwrap_or(0.5);
            let sb = self.arms.get(&(intent.clone(), (*b).clone())).map(|p| p.mean()).unwrap_or(0.5);
            sa.partial_cmp(&sb).unwrap_or(std::cmp::Ordering::Equal)
        }).unwrap().clone()
    }
    ...
    pub fn detect_intent(query: &str) -> QueryIntent {
        ...
        if q.contains("yesterday") || q.contains("last week") || q.contains("ago") || q.contains("when") {
            QueryIntent::Temporal
        } else if q.contains(".cpp") || q.contains(".rs") || q.contains(".py") || q.contains("fn ") || q.contains("impl ") {
            QueryIntent::FileRef
        } else if q.contains("wrong") || q.contains("correct") || q.contains("actually") || q.contains("mistake") {
            QueryIntent::Correction
        } else if q.contains("rust") || q.contains("function") || q.contains("struct") || q.contains("code") {
            QueryIntent::Code
        } else {
            QueryIntent::General
        }
