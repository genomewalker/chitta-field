...
// EMBED_DIM / EMBED_MODEL_ID / EMBED_MODEL_ID_CSTR / TEXT_FORMAT_VERSION are generated by
// build.rs from build-time env (CHITTA_EMBED_DIM / CHITTA_EMBED_MODEL_ID /
// CHITTA_TEXT_FORMAT_VERSION), defaulting to the public-shipped nomic-embed-text-v1.5
// (768-dim) so the CI release matches its bundled model. Personal/local builds set those
// env vars (the C++ CMake reads the same ones) so one environment drives both layers.
// These feed StoreHeader.vector_space_id = FNV-1a(model_id, embed_dim, text_format) and
// are the single source of truth for the in-process embedder and re_embed metadata.
include!(concat!(env!("OUT_DIR"), "/embed_config.rs"));
